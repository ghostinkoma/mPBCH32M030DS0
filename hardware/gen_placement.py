...
PARTS = [
    # --- 上辺: 信号系コネクタ / 操作 ---
    (2, 0.5, 8, 6, "ui", "D7 D2 D8", "電源LED"),
    (12, 1, 6, 6, "ui", "SW1", "RESET"),
    (19.5, 1, 6, 6, "ui", "SW2", "USER"),
    (26.5, 2, 2.5, 2.5, "ui", "D3", ""),
    (31, 0.5, 11, 3.5, "con", "J7 SDI", "4P"),
    (44, 0, 7, 5, "con", "J4 I2C", "SH4"),
    (53, 0, 13, 6, "con", "J5 UART", "XH4"),
    (68, 1, 12, 5, "ana", "JP8 / C105", "QII 入力"),
    # --- 右辺: GPIO 引き出し ---
    (84.5, 8, 5, 21, "con", "J6", "GPIO 8P"),
    # --- MCU まわり ---
    (27, 16, 4, 3.5, "mcu", "Y1", "8MHz"),
    (36, 16, 6, 6, "mcu", "U1", "QFN48"),
    (21, 10, 7, 4.5, "pwr", "D5/D6", "R10 VHV"),
    (50, 20, 8, 3, "ana", "JP5", "ISEL_B"),
    (50, 24, 8, 3, "ana", "JP7", "ISEL_A"),
    (62, 21.5, 20, 4, "ana", "JP2-JP4", "HALL/BEMF 選択"),
    # --- 左辺: USB-C と PD 給電パス (大電流 5A は下半分へ直行) ---
    (0, 15, 8, 9.5, "con", "J8 USB-C", "左向き"),
    (9, 15, 4, 4, "mcu", "U3", "ESD"),
    (9, 20, 5, 3, "pwr", "F2/F3", ""),
    (9, 29.5, 11, 6, "pwr", "U5+Q10", "PD 給電"),
    # --- 電源入力 (左辺・左下) ---
    (0, 44, 8, 11, "con", "J1", "PWR IN"),
    (11, 38, 9, 5, "pwr", "U2 78L05", "+5V"),
    (10, 51, 11, 5, "pwr", "F1 15A", ""),
    (10, 44, 11, 5, "pwr", "U4+Q9", "理想ダイオード"),
    (22, 44, 5, 5, "pwr", "D1", "TVS"),
    # --- バス電流 ---
...
def main():
    ...
    pd_path = [(8, 20), (14.5, 29.5), (14.5, 36), (24, 42)]
    d2 = " ".join(("M" if i == 0 else "L") + f"{MX + x * S:.0f},{MY + y * S:.0f}" for i, (x, y) in enumerate(pd_path))
    o.append(f'<path d="{d2}" fill="none" stroke="#d35400" stroke-width="2.5" stroke-dasharray="6,4" '
             f'marker-end="url(#ar)" opacity="0.8"/>')
    o.append(text(21, 37.3, "USB-PD 5A", 8, "#d35400", "start"))
    ...
    o.append(f'<path d="{d}" fill="none" stroke="#c0392b" stroke-width="2.5" stroke-dasharray="10,5" '
             f'marker-end="url(#ar)" opacity="0.8"/>')
    o.append(text(22, 27.0, "大電流: J1→F1→Q9→C1/C2→HB→R70→GND", 9, "#c0392b", "start"))
    # スター GND / ケルビン
    ...
    notes = [
        "端子台の配置",
        "・J1 電源 (2P) と USB-C は左辺, J2 モータ (4P) は下辺",
        "  → 別の辺に分けて誤接続を防ぐ",
        "・USB-PD 給電 (≤5A) は U5+Q10 から左下の",
        "  VBUS へ直行し, ロジック領域を横切らない",
