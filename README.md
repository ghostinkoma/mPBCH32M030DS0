...
> **ステータス: Rev 0.4 — 回路図 + 2 層基板レイアウト (未試作。MCU モジュールは手配線の仕上げが残っています: 8 マス版 22 か所 / 9 マス版 8 か所)**。無保証です (LICENSE)。
...
MCU モジュールの幅を 2 通り用意しました。回路図・ピン配置・ファームウェアは共通で、基板の幅だけが違います。
子基板もモジュールの端子列に合わせて 2 通りあります (8 マス版のモジュールには 8 マス版の子基板を組み合わせます)。

| | **8 マス版** (`hardware/`, `hardware/daughter/`) | **9 マス版** (`hardware/w9/`, `hardware/w9/daughter/`) |
|---|---|---|
| モジュール | 20.32 × 53.34mm (端子列 0.6in) | 22.86 × 53.34mm (端子列 0.7in, ブレッドボードの両側に 2 列ずつ空く) |
| モジュールの自動配線 | **未接続 22 か所** (手配線が必要) | **未接続 8 か所** (手配線が必要) |
| 手配線用 (ベタなし) | `hardware/w8_nopour/` | `hardware/w9_nopour/` |
| 子基板 A / C | 20.32 × 71.94mm (−42%), 未接続 0 | 22.86 × 70.74mm (−36%), 未接続 0 |
| 子基板 B | 40.64 × 66.94mm (−38%), 未接続 0 | 43.18 × 66.94mm (−34%), 未接続 0 |
| 製造データ / 図 / DRC | `hardware/fab/*.zip`, `docs/pcb/` | `hardware/fab/w9/*.zip`, `docs/pcb/w9/` |

- 「ベタなし」版は、自動配線の配線とビアを残して GND ベタ・大電流の太らせたベタを外したものです (ベタへ落とすだけの
  スティッチングビアも外しています)。手配線で仕上げてから、KiCad でベタ (GND 両面) を引き直してください。
- 9 マス版は、8 マス版を手配線で仕上げるときの足がかり (配線の通し方の参考) にもなります。
- 9 マス版の子基板は幅が広がるぶん、面積の削減率が 40% を下回ります (上表)。
- 再生成: `MPB_W9=1 python3 gen_pcb.py all` (9 マス版)、`python3 gen_pcb.py nopour` / `MPB_W9=1 python3 gen_pcb.py nopour` (ベタなし版)。
...
| `hardware/w9/`, `hardware/w9_nopour/`, `hardware/w8_nopour/` | 9 マス版 (モジュール + 子基板)、手配線用のベタなし版 (上の「8 マス版と 9 マス版」) |
...
| 基板 (2 層) | **子基板 A/B/C: 未接続 0 / 電気的 DRC エラー 0** (シルクの重なり等の警告のみ)。**MCU モジュール: 自動配線で 8 マス版 22 か所 / 9 マス版 8 か所が未接続 → KiCad で手配線して仕上げが必要** ([docs/stacking.md の残作業](docs/stacking.md#mcu-モジュールの残作業-手配線))。製造データ (ガーバー・ドリル・部品座標) は `hardware/fab/*.zip` (モジュールは配線完了まで発注不可) |
