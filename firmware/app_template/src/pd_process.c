/*
 * USB PD シンク (WCH CH32M030 EVT "USBPD_SNK/PD_Process" を mPBCH32M030DS0 向けに改変)
 * Modified 2026 by ghostinkoma:
 *  - PD0 (CC1/CC2 = PA0/PA1) を使用
 *  - 要求 PDO をポリシー関数 Mpb_PD_SelectPdo() で選択 (元: 常に 5V)
 *  - PS_RDY 受信 / 切断で Mpb_PD_OnPsRdy() / Mpb_PD_OnDetach() を呼び、給電パスを制御
 *  - printf を PD_LOG (既定: 無効) に置換, 1ms 時間基準は TIM1 ではなく mpb_time (TIM3)
 */
/********************************** (C) COPYRIGHT *******************************
...
#if(PD_SEL==PD0)
void USBPD0_IRQHandler(void) MPB_IRQ;
#elif(PD_SEL==PD1)
void USBPD1_IRQHandler(void) MPB_IRQ;
#endif
...
#if(PD_SEL==PD0)
        NVIC_EnableIRQ( USBPD0_IRQn );
#elif(PD_SEL==PD1)
        NVIC_EnableIRQ( USBPD1_IRQn );
#endif
...
void PD_SINK_Init( )
{
    ...
    USBPD->PORT_CC1 = CC_CVS_66 | CC_PD;
    USBPD->PORT_CC2 = CC_CVS_66 | CC_PD;
...
void PD_Init( void )
{
    ...
    PWR->CTLR|=(1<<10); //Enable ISINK
    RCC_PB2PeriphClockCmd(RCC_PB2Periph_GPIOA, ENABLE);               /* Open PD I/O clock, AFIO clock and PD clock */
#if(PD_SEL==PD0)
    RCC_HBPeriphClockCmd( RCC_HBPeriph_USBPD0, ENABLE );
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1;
#elif(PD_SEL==PD1)
    RCC_HBPeriphClockCmd( RCC_HBPeriph_USBPD1, ENABLE );
...
UINT8 PD_Detect( void )
{
    ...
    {
        USBPD->PORT_CC1 &= ~( CC_CE|CC_CVS_Mask );
        USBPD->PORT_CC1 |= CC_CVS_22;
        USBPD->PORT_CC1|=CC_CE;
        Delay_Us(2);
        if( USBPD->PORT_CC1 & CC_CMPO )
        {
            cmp_cc1 |= bCC_CMP_22;
        }

        USBPD->PORT_CC2 &= ~( CC_CE|CC_CVS_Mask );
        USBPD->PORT_CC2 |= CC_CVS_22;
        USBPD->PORT_CC2|=CC_CE;

        Delay_Us(2);
        if( USBPD->PORT_CC2 & CC_CMPO )
        {
            cmp_cc2 |= bCC_CMP_22;
        }
...
UINT8 PD_Send_Handle( UINT8 *pbuf, UINT8 len )
{
    ...
    {
#if(PD_SEL==PD0)
        NVIC_DisableIRQ( USBPD0_IRQn );
#elif(PD_SEL==PD1)
        NVIC_DisableIRQ( USBPD1_IRQn );
#endif
    ...
    if( PD_Ctl.Flag.Bit.Msg_Recvd )
    {
        ...
        switch( pd_header )
        {
            case DEF_TYPE_SRC_CAP:
                ...
                PDO_Request( Mpb_PD_SelectPdo( &Adapter_SrcCap[ 1 ], PDO_Len ) );
                break;
